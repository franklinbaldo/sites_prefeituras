...
export async function originalFetchPSI(url, apiKey, fetchFn) {
  ...
  // Check for incomplete or error response from PSI API
  if (json.error || !json.lighthouseResult || !json.lighthouseResult.categories) {
    const errorMessage = `Incomplete or error response from PSI API for ${url}: ${JSON.stringify(json)}`;
    logMessage('ERROR', errorMessage, 'fetchPSIErrorResponse');
    throw new Error(`PSI API returned incomplete or error response for ${url}`);
  }

  // const cat = json.lighthouseResult.categories; // This line is no longer strictly needed
  return {
    url,
    performance: json.lighthouseResult?.categories?.performance?.score ?? null,
    accessibility: json.lighthouseResult?.categories?.accessibility?.score ?? null,
    seo: json.lighthouseResult?.categories?.seo?.score ?? null,
    bestPractices: json.lighthouseResult?.categories?.['best-practices']?.score ?? null,
