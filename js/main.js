...
function renderTable(data) {
  ...
  data.forEach((item) => {
    ...
    bestPracticesCell.textContent = item.bestPractices
      ? (item.bestPractices * 100).toFixed(0) + "%"
      : "N/A";
...
function initializeFiltering() {
  ...
  input.addEventListener("keyup", () => {
    ...
    currentData = psiData.filter(
      (item) =>
        item.url.toLowerCase().includes(q) ||
        String(item.ibge_code || "").includes(q),
    );
    renderTable(currentData);
  });
}
...
async function initializeApp() {
  ...
  if (psiData) {
    console.log(
      "PSI Data fetched successfully in main.js:",
      psiData.length,
      "records",
    );
    currentData = [...psiData];
...
function dataToCsv(data) {
  ...
  const lines = data.map((item) =>
    header.map((h) => (item[h] !== undefined ? item[h] : "")).join(","),
  );
  return header.join(",") + "\n" + lines.join("\n");
}
