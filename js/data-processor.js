    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.error("Error fetching PSI results: data/psi-latest-viewable-results.json not found. The GitHub Action might not have run successfully yet or the file was not generated.");
          ...
          if (tableBody) {
            tableBody.innerHTML = '<tr><td colspan="6">Os dados ainda não estão disponíveis. Por favor, verifique mais tarde. A rotina de coleta de dados pode não ter sido executada ainda.</td></tr>';
          }
        } else {
      ...
      const tableBody = document.getElementById('psi-results-table').getElementsByTagName('tbody')[0];
      if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="6">Erro ao carregar os dados. Por favor, tente recarregar a página.</td></tr>';
      }
